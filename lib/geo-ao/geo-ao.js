// just a copy of rye terrell's geo-ambient-occlusion library, so I try it with some edits
...
module.exports = function(positions, opts) {
  ...
  function sample() {
    ...
    mat4.rotateX(model, model, Math.pow(Math.random() * 2 - 1, 2) * Math.PI * 0.5);
    // mat4.rotateY(model, model, (Math.random() * 2 - 1) * Math.PI * 0.35);
    mat4.rotateZ(model, model, Math.random() * Math.PI * 2);
