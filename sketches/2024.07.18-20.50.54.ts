...
async function main() {
  ...
  const { indexCount, buffers, extent } = result
   ...
   * - Then, figure out how to do CPU-based culling of the groups
   * ------
   * - Then, figure out how to do GPU-based culling of the groups
   */
  // --------------------------------------------------------

  const drawCalls = 50
  const indexGroups = getIndexGroups(drawCalls, result.data.positions, result.data.indexes)
  const indirectDrawParams = new Uint32Array(5 * drawCalls)
  for (let i = 0; i < indexGroups.length; i++) {
    const indexGroup = indexGroups[i]
    indirectDrawParams[i * 5 + 0] = indexGroup.indexCount // The indexCount value
    indirectDrawParams[i * 5 + 1] = 1 // The instanceCount value
    indirectDrawParams[i * 5 + 2] = indexGroup.indexOffset // The firstIndex value
    indirectDrawParams[i * 5 + 3] = 0 // The baseVertex value
    indirectDrawParams[i * 5 + 4] = 0 // The firstInstance value
  }

  const indirectBuffer = createGPUBuffer(device, indirectDrawParams.buffer, GPUBufferUsage.INDIRECT | GPUBufferUsage.COPY_DST)
  device.queue.writeBuffer(indirectBuffer, 0, indirectDrawParams, 0, indirectDrawParams.length)
  ...
  let curIdxGroup = 0
  setInterval(() => {
    curIdxGroup = (curIdxGroup + 1) % drawCalls
  }, 150)
  requestAnimationFrame(function loop() {
    ...
    const isDirty = isCameraMoving || lastIdxGroup !== curIdxGroup
    lastIdxGroup = curIdxGroup
    ...
    if (isDirty) {
      ...
      // for (let i = 0; i < drawCalls; i++) {
        mainRenderPass.drawIndexedIndirect(indirectBuffer, 20 * curIdxGroup)
      // }
      renderBboxGroups(mainRenderPass)
      mainRenderPass.end()
...
function createBoundingBoxRenderer(device: GPUDevice, indexGroups: IndexGroup[], bindGroupLayout: GPUBindGroupLayout) {
  ...
  return function renderBboxGroups(renderPass: GPURenderPassEncoder) {
...
function frustumIntersectsBox(box: BoundingBox, viewProjectionMatrix: Float32Array): boolean {
  // Extract the planes from the view-projection matrix
  const planes = extractPlanesFromMatrix(viewProjectionMatrix)
  ...
  // Check if the box is outside any of the frustum planes
  for (const plane of planes) {
    if (corners.every(corner => isPointOutsidePlane(corner, plane))) {
      return false
    }
  }
...
type Plane = { normal: [number, number, number], distance: number }

function extractPlanesFromMatrix(matrix: Float32Array): Plane[] {
  const planes: Plane[] = []
  for (let i = 0; i < 6; i++) {
    const row = Math.floor(i / 2)
    const sign = i % 2 === 0 ? 1 : -1
    const normal = {
        x: matrix[row * 4 + 0] * sign,
        y: matrix[row * 4 + 1] * sign,
        z: matrix[row * 4 + 2] * sign
    }
    const distance = matrix[row * 4 + 3] * sign
...
function isPointOutsidePlane(point: [number, number, number], plane: Plane): boolean {
  return (
