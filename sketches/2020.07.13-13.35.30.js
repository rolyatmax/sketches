...
gui.add(settings, 'footprintScale', 0, 4).step(0.01).onChange(onChangeBuildingScale)
...
fetch('resources/data/nyc-buildings/indexed-building-triangles.bin')
...
function onLoadData (binaryData) {
  ...
  const draw = rico({
    ...
    void main() {
      float angle = noise3D(buildingCentroid, noiseFreq * 20.0, vec2(0.0, 6.283));
      vec3 offset = vec3(cos(angle), sin(angle), 0) * 0.001;
      ...
      float colorT = noise3D(
        vec3(buildingCentroid.xy * 3.0, time * 0.05) + vec3(10),
        noiseFreq,
        vec2(grayColor - colorRange * 0.5, grayColor + colorRange * 0.5)
      );
      vec3 lightDir = normalize(vec3(1));
      color = getColorFromPalette(colorT + dot(lightDir, norm) * normMult);
