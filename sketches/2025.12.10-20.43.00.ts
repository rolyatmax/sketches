// Same as previous sketch, but with a perspective transform
...
const PLOTNAME = '2025.12.10-20.43.00'
  ...
  canvasSketch(({ render }) => {
    ...
    return (args: SketchArgs) => {
      ...
      for (let i = 0; i < numSpheres; i++) {
        ...
        const aspect = 1 // Square aspect for each sphere cell if we want 1:1, but here we just project into unit cube
        ...
        const line2d: Line2D = baseLine3d.map(pt => {
          ...
          // Project using perspective matrix
          const projected = vec3.create()
          vec3.transformMat4(projected, rotatedPt, viewProjection)
          
          // Map from normalized device coordinates (-1 to 1) to screen coordinates
          // centered at (cx, cy) with radius scale
          const x = projected[0] * sphereRadiusScale + cx
          const y = -projected[1] * sphereRadiusScale + cy // flip Y because canvas Y is down
