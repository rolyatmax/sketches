...
let nOffset, renderCube, curAO, positions, normals, setup
...
gui.add(settings, 'meshCount', 1, 10000).step(1).onChange(onChange)
...
gui.add(settings, 'capsuleResolution', 1, 16).step(1).onChange(onChange)
gui.add(settings, 'capsuleHeightScale', 0, 20).step(0.01).onChange(onChange)
gui.add(settings, 'capsuleSegments', 3, 24).step(1).onChange(onChange)
gui.add(settings, 'size', 0, 2).step(0.01).onChange(onChange)
gui.add(settings, 'freq', 0, 0.1).step(0.001)
gui.add(settings, 'sizeFreq', 0, 0.1).step(0.001).onChange(onChange)
...
const sketch = ({canvas}) => {
    ...
    renderCube = regl({
      vert: glsl`
        ...
        void main (void) {
          ...
          float d = snoise(vec4((position + nOffset.yyz) * nFreq, frame * speed)) * 0.5 + 0.5;
          ...
          fragColor.rgb *= pow(1.0 - curAO + 0.1, 0.8);
  ...
  function updatePositions (spreadSize) {
    ...
    const mesh = primitiveCapsule(settings.size, settings.size * settings.capsuleHeightScale, settings.capsuleResolution, settings.capsuleSegments)
  ...
  return () => {
    ...
    if (settings.camera) {
      camera.center = [
        Math.sin(time * 0.3) * 100 * settings.distance,
        Math.cos(time * 0.4) * 90 * settings.distance,
        Math.sin((time + 10) * 0.5) * 85 * settings.distance
      ]
    }
...
function getGeometry (mesh, meshCount, spreadSize) {
  ...
  let j = meshCount
  while (j--) {
    const offset = random.insideSphere(spreadSize)
    const size = 5 * (random.noise3D(offset[0], offset[1], offset[2], settings.sizeFreq))
    if (size <= 0) continue
    // const quat = random.quaternion()
    const quat = [
      random.noise3D(offset[0] + 1000, offset[1] + 1000, offset[2] + 1000, settings.sizeFreq * 0.3),
      random.noise3D(offset[0] + 1000, offset[1] + 200, offset[2] + 400, settings.sizeFreq * 0.3),
      random.noise3D(offset[0] + 1000, offset[1] + 600, offset[2] + 7000, settings.sizeFreq * 0.3),
      random.noise3D(offset[0], offset[1] + 800, offset[2] + 200, settings.sizeFreq * 0.3)
    ]
    const transform = p => vec3.transformQuat([], p, quat)
    mesh.cells.forEach((tri) => {
      for (let i = 0; i < 3; i++) {
        positions.push(
          transform(mesh.positions[tri[i]])[0] * size + offset[0],
          transform(mesh.positions[tri[i]])[1] * size + offset[1],
          transform(mesh.positions[tri[i]])[2] * size + offset[2]
        )
      }
    })
  }
