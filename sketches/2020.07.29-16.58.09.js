...
gui.add(settings, 'diffuse', 0, 5).step(1)
...
gui.add(settings, 'pixelSize', 0.001, 0.1).step(0.001)
...
function setup () {
  ...
  renderTrails = rico({
    ...
    void main() {
      vec2 pos = vUV;
      float z = texture(trailMap, pos).x;
      vec3 color1 = getColor(texture(trailMap, pos + colorShift).x);
      vec3 color2 = getColor(texture(trailMap, pos - colorShift).x);
      vec3 color3 = getColor(z);
  
      float z1 = texture(trailMap, pos + vec2(pixelSize, 0)).x;
      float z2 = texture(trailMap, pos + vec2(0, pixelSize)).x;
      ...
      float shade = clamp(dot(n, lightDir), 0.0, 1.0);
