...
gui.add(settings, 'gridSize', 2, 50).step(1) // .onChange(setup)
gui.add(settings, 'threshold', -1, 1) // .onChange(setup)
gui.add(settings, 'noiseFreq', 0, 0.1) // .onChange(setup)
gui.add(settings, 'speed', 0.1, 100)
gui.add(settings, 'isolines', 1, 100).step(1)
gui.add(settings, 'opacity', 0, 1).step(0.01)
gui.add(settings, 'lineWidth', 0.1, 5)
    ...
    for (let x = xOffset; x < WIDTH; x += settings.gridSize) {
      for (let y = yOffset; y < HEIGHT; y += settings.gridSize) {
        for (let i = 0; i < isolines.length; i++) {
          ...
          lineGroups[i].push(...getLinesForSquare(x, y, time * settings.speed, settings.gridSize, threshold))
        }
      }
    }
...
function getLinesForSquare (x, y, z, gridSize, threshold) {
  const x1 = x - gridSize / 2
  const x2 = x + gridSize / 2
  const y1 = y - gridSize / 2
  const y2 = y + gridSize / 2

  const p1 = Number(noise(x1, y1, z) < threshold)
  const p2 = Number(noise(x2, y1, z) < threshold)
  const p3 = Number(noise(x2, y2, z) < threshold)
  const p4 = Number(noise(x1, y2, z) < threshold)
