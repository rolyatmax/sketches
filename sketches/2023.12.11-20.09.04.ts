...
const PLOTNAME = '2023.12.11-20.09.04'

const MM_PER_INCH = 25.4
const PIXELS_PER_INCH = 200
const WIDTH = 4.5 * PIXELS_PER_INCH
const HEIGHT = 6.25 * PIXELS_PER_INCH
...
canvasSketch(({ render }) => {
  ...
  return (args: SketchArgs) => {
    ...
    settings.boxes = rand.rangeFloor(5, 40)
    settings.minHeight = Math.floor(Math.pow(rand.value(), 6) * 15 + 2)
    settings.lineHeight = Math.floor(Math.pow(rand.value(), 3) * 10 + 10)
