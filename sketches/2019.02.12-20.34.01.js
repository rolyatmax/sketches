...
gui.add(settings, 'pointSize', 0, 250)
...
gui.add(settings, 'accel', 0.00001, 0.001).step(0.00001)
gui.add(settings, 'friction', 0, 0.2).step(0.001)
...
function changeNoise () {
  noiseSpring.moveTo(rand.range(settings.noiseMag / 50))
  hueSpreadSpring.moveTo(rand.range(settings.hueSpread))
  hueStartSpring.moveTo(rand.value())
  sizeSpring.moveTo(rand.range(0.5, 1) * settings.pointSize)
}
...
const sketch = ({ gl }) => {
  ...
  setup = function setup () {
    ...
    const points = new Array(settings.pointCount).fill(null).map(() => ({
      position: settings.onSphere ? rand.onSphere() : rand.insideSphere()
    }))
    ...
    drawCirclesModel = new luma.Model(gl, {
      vs: `#version 300 es
        ...
        void main() {
          ...
          float sat = snoise(vec4((position + nOffset.xzy) * nFreq, frame / 500.0));
          ...
          pointColor = hsl2rgb(vec3(hue, saturation + saturationOffset, lightness + lightnessOffset));
        }
      `,
      fs: `#version 300 es
        precision highp float;
  ...
  return () => {
    ...
    drawCirclesModel.draw({
      uniforms: {
        ...
        sizeMultiplier: sizeSpring.tick(settings.accel, settings.friction),
        noiseMultiplier: noiseSpring.tick(settings.accel, settings.friction),
        saturation: settings.saturation,
        lightness: settings.lightness,
        hueStart: hueStartSpring.tick(settings.accel, settings.friction),
        hueSpread: hueSpreadSpring.tick(settings.accel, settings.friction),
        nOffset: nOffset,
        frame: frame,
        nFreq: settings.freq
      }
