...
function setup () {
  ...
  const center = mesh.positions.reduce((av, pt) => [
  ...
  for (const triangles of meshes) {
    ...
    const rotationOffset = vec3.add([], rand.insideSphere(10), center)
...
const draw = rico({
  ...
  void main() {
    ...
    vColor = vec4(color, 1);
    vec3 p = transform(position - rotationOffset, quat) + rotationOffset;
...
const sketch = () => {
  ...
  return ({ width, height, time }) => {
    if (settings.roam) {
      camera.up = [0, 1, 0]
      camera.center = [settings.cameraDist * Math.sin(time / 10), settings.cameraDist * Math.cos(time / 10), 0]
    }
    camera.tick()
