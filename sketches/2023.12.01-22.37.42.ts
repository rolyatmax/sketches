...
canvasSketch(({ render }) => {
  ...
  return (args: SketchArgs) => {
    ...
    const n = 15
    shapes.push([
      [[margin - n, margin - n], [margin + rectWidth + n, margin - n]],
      [[margin + rectWidth + n, margin - n], [margin + rectWidth + n, margin + rectHeight + n]],
      [[margin + rectWidth + n, margin + rectHeight + n], [margin - n, margin + rectHeight + n]],
      [[margin - n, margin + rectHeight + n], [margin - n, margin - n]]
    ])
    ...
    while (loops--) {
      ...
      vec2.add(initOffset, initOffset, vec2.scale([], vec2.random([]), rand.range(50)))

      let offset = initOffset.slice()
      let lineCount = 0
      while (true) {
        const line: Line = [
        ...
        vec2.add(offset, offset, vec2.scale([], perpDir, rand.range(settings.minDist, settings.maxDist)))
