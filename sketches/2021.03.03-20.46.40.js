...
const sketch = ({ render }) => {
  ...
  return ({ context, width, height }) => {
    ...
    console.log('triangles:', lines1.length + lines2.length)
...
window.toSVG = function () {
  ...
  console.log(polylinesToSVG(lines1.map(line => line.map(pt => pt.map(v => v / PIXELS_PER_CM))), opts))
  console.log(polylinesToSVG(lines2.map(line => line.map(pt => pt.map(v => v / PIXELS_PER_CM))), opts))
}
