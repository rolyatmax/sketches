...
async function main() {
   ...
   * - figure out how to do GPU-based culling of the groups
   */
  // --------------------------------------------------------
  ...
  requestAnimationFrame(function loop() {
    ...
    if (isDirty) {
      ...
      const viewProjMatrix = mat4.multiply(mat4.create(), projMatrix, view)

      for (let i = 0; i < indexGroups.length; i++) {
        // for (let i = indexGroups.length - 1; i >= 0; i--) {
        const indexGroup = indexGroups[i]
        const isVisible = intersectsBox(viewProjMatrix, [indexGroup.min, indexGroup.max])
        // console.log({ isVisible })
        indirectDrawParams[i * 5 + 0] = isVisible ? indexGroup.indexCount : 0 // The indexCount value
      ...
      for (let i = 0; i < drawCalls; i++) {
        mainRenderPass.drawIndexedIndirect(indirectBuffer, 20 * i)
      }
