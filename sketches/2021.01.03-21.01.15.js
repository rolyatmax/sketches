  ...
  return ({ context, width, height }) => {
    ...
    for (const c of circles) {
      ...
      if (settings.showCenters) {
        ...
        const val2 = rand.noise2D(c.x + 100, c.y, 0.0004)
        const chance1 = Math.abs(rand.noise2D(c.x + 900, c.y + 900, 0.0005))
        if (rand.chance(chance1 * 0.1)) {
          context.lineWidth = c.r * 0.5
          radius -= context.lineWidth / 2
          const idx1 = 4 * (val1 * 0.5 + 0.5) | 0
          const idx2 = 4 * (val2 * 0.5 + 0.5) | 0

          const chance2 = Math.abs(rand.noise2D(c.x + 500, c.y, 0.0003))
          context.strokeStyle = rand.chance(chance2) ? colors[idx1] : colors[idx2]
        } else {
          context.lineWidth = 1
