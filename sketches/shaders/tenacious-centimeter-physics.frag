...
void main() {
  ...
  if (prevPosition == currPosition || prevPosition == vec3(0.0, 0.0, 0.0)) {
    vec3 randVelocity = vec3(rand(currState.xw), rand(currState.yw * 30.0), rand(currState.zw * 20.0)) * excitability;
    prevPosition = randVelocity + currPosition;
  }

  vec3 velocity = currPosition - prevPosition;

  vec3 center = vec3(cos(tick / 145.0 + 20.0), sin(tick / 300.0), sin(tick / 300.0 + 10.0));
  float dist = distance(center, currPosition);
  vec3 pull = (center - currPosition) * pow(1.0 - dist, pullStrength) * excitability;

  vec3 center2 = 0.2 * vec3(sin(tick / 145.0 + 200.0), -cos(tick / 100.0 + 20.0), sin(tick / 200.0 + 1.0));
  float dist2 = distance(center2, currPosition);
  vec3 pull2 = (center2 - currPosition) * pow(1.0 - dist2, pullStrength) * excitability;

  vec3 center3 = 0.5 * vec3(cos(tick / 55.0 + 300.0), -cos(tick / 10.0 + 50.0), sin(tick / 230.0) + 2.0);
  float dist3 = distance(center3, currPosition);
  vec3 pull3 = (center3 - currPosition) * pow(1.0 - dist3, pullStrength) * excitability;

  vec3 pull4 = vec3(0.0, -0.00002, 0.0);

  velocity = velocity + pull + pull2 + pull3 + pull4;
  velocity = velocity * (1.0 - decay);
