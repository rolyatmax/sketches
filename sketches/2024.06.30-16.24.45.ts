...
async function main() {
  ...
  const pickingDepthTexture = device.createTexture({
    size: { width: canvas.width, height: curTexture.height },
    format: 'depth24plus',
  ...
  const buildingColors = new Uint8Array(buildingIdsArray.length * 4)
  for (let i = 0; i < buildingIdsArray.length; i++) {
    const buildingId = buildingIdsArray[i]
  ...
  fn mainVertex(
    ...
    @location(1) buildingColor: vec4<f32>
  ) -> Output {
    let p = position;
    var output: Output;
    output.color = buildingColor;
...
const pipeline = device.createRenderPipeline({
  ...
  requestAnimationFrame(function loop() {
    ...
    const isDirty = !mat4.equals(viewprojData.subarray(16), view) || !mat4.equals(viewprojData, projMatrix)
    isCameraMoving = isDirty
    ...
    } else if (hoveredPixel && pickingBuffer.mapState === 'unmapped') {
      const commandEncoder = device.createCommandEncoder()
    ...
    console.log('building ID:', hoveredBuildingId)
