...
function onLoadData ([lidarBinaryData, buildingBinaryData]) {
  ...
  const buildingsVertexArray = rico.createVertexArray()
    .vertexAttributeBuffer(0, rico.createVertexBuffer(rico.gl.FLOAT, 3, buildingBuffers.positions))
    .vertexAttributeBuffer(1, rico.createVertexBuffer(rico.gl.FLOAT, 3, buildingBuffers.normals))
  ...
  const drawBuildings = rico({
    ...
    count: buildingBuffers.positions.length / 3,
    primitive: 'triangles',
    vs: injectGLSL(NOISE_GLSL, PALETTE_ANIMATOR_GLSL, `#version 300 es
    precision highp float;
