...
const PLOTNAME = '2023.12.01-22.37.42'
...
canvasSketch(({ render }) => {
  ...
  gui.add(settings, 'loops', 0, 20).step(1).onChange(render)
  gui.add(settings, 'lineNoiseFreq', 0, 10).onChange(render)
  ...
  return (args: SketchArgs) => {
    ...
    function perturbLine (line: Line): Shape {
      const [start, end] = line
      const dir: Vec2 = vec2.sub([], end, start)
      ...
      for (let i = 0; i <= divisions; i++) {
        const t = i / divisions
        const pt = vec2.lerp([], start, end, t)
        const mag = rand.noise1D(t + tStart, settings.lineNoiseFreq, settings.lineNoiseMag)
        vec2.add(pt, pt, vec2.scale([], norm, mag))
        if (prevPt !== null) {
          shape.push([[prevPt[0], prevPt[1]], [pt[0], pt[1]]])
        }
        prevPt = pt
      }
    ...
    const mainShape: Shape = [
      [[margin, margin], [margin + rectWidth, margin]],
      [[margin + rectWidth, margin], [margin + rectWidth, margin + rectHeight]],
      [[margin + rectWidth, margin + rectHeight], [margin, margin + rectHeight]],
      [[margin, margin + rectHeight], [margin, margin]]
    ]

    // extend the rectangle out by n pixels
    const n = 15
    shapes.push(perturbLine([[margin - n, margin - n], [margin + rectWidth + n, margin - n]]))
    shapes.push(perturbLine([[margin + rectWidth + n, margin - n], [margin + rectWidth + n, margin + rectHeight + n]]))
    shapes.push(perturbLine([[margin + rectWidth + n, margin + rectHeight + n], [margin - n, margin + rectHeight + n]]))
    shapes.push(perturbLine([[margin - n, margin + rectHeight + n], [margin - n, margin - n]]))

    let loops = settings.loops
    while (loops--) {
      const angle = rand.range(Math.PI * 2)
      const dir: Vec2 = [Math.cos(angle), Math.sin(angle)]

      const perpDir: Vec2 = [dir[1], -dir[0]]

      const initOffset: Vec2 = [width / 2, height / 2]
      vec2.add(initOffset, initOffset, rand.onCircle(rand.range(50)))

      ;[-1, 1].forEach(mult => {
        const offset = initOffset.slice()
        let lineCount = 0
        if (mult === -1) vec2.add(offset, offset, vec2.scale([], perpDir, rand.range(mult * settings.minDist, mult * settings.maxDist)))
        while (true) {
          const line: Line = [
            vec2.scaleAndAdd([], offset, dir, -1000),
            vec2.scaleAndAdd([], offset, dir, 1000)
          ]
          vec2.add(offset, offset, vec2.scale([], perpDir, rand.range(mult * settings.minDist, mult * settings.maxDist)))

          const intersections = shapeLineIntersections(line, mainShape)
          lineCount += 1
          if (!intersections.length || lineCount > 100000) break
          shapes.push(perturbLine([intersections[0], intersections[1]]))
        }
      })
    }
