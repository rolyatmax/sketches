...
const sketch = ({ render }) => {
  ...
  gui.add(settings, 'boxes', 1, 200).step(1).onChange(render)
  ...
  return (args) => {
    ...
    const circle = {
      pos: [rand.range(width), rand.range(height)],
      radius: width * 0.1
    }

    const h = settings.boxHeight * width
    for (let i = 0; i < settings.boxes; i++) {
      const y = margin + (height - margin * 2) * i / (settings.boxes - 1) - h / 2
      let x = margin
      let lineStart = x
      while (x < width - margin) {
        ...
        boxes.push([[lineStart, yStart], [x, yEnd], [x, yEnd + h], [lineStart, yStart + h], [lineStart, yStart]])
    ...
    boxes = boxes
      .filter(line => !boxCircleIntersect(line, circle))
      .map(line => handdraw(line, settings.handdrawGranularity * width, rand, settings.handdrawFreq, settings.handdrawMag * width))
    ...
    console.log('boxes count:', boxes.reduce((tot, line) => line.length + tot, 0))
  }
}
...
function boxCircleIntersect (line, circle) {
  // very hacky way to do this - just take the bounding box of the line
  // and see if that intersects with the circle
  // this is fine for this particular use case because the lines _are_ bounding boxes
  ...
  const rx = min[0]
  const ry = min[1]
  const rw = max[0] - min[0]
  const rh = max[1] - min[1]
