...
const sketch = () => {
  ...
  function createCircles () {
    ...
    const keys = (a, b) => [[a, b].join('|'), [b, a].join('|')]
    ...
    for (let i = 0; i < triangulatedPositions.length; i += 3) {
      ...
      const sets = [[pos1, pos2, pos3], [pos2, pos3, pos1], [pos3, pos1, pos2]]
      for (const s of sets) {
        for (const k of keys(s[0], s[1])) {
          const thirdPts = triangleMap.get(k) || []
          thirdPts.push(s[2])
          triangleMap.set(k, thirdPts)
        }
      }

      const positions = rand.pick(sets)

      triangles.push({
        triScale: createSpring(0.04, 0.15, 0),
        positions,
        thirdPtT: createSpring(0.02, 0.2, 0)
      })
    }

    for (const t of triangles) {
      const k = keys(t.positions[0], t.positions[1])[0]
      const pt = triangleMap.get(k).filter(p => p !== t.positions[2])[0]
      t.thirdPt = pt || t.positions[2]
    }
  ...
  return ({ context, width, height, frame }) => {
    ...
    for (const t of triangles) {
      const positions = [
        t.positions[0],
        t.positions[1],
        vec2.lerp([], t.positions[2], t.thirdPt, t.thirdPtT.getCurrentValue())
      ]

      const center = t.positions.reduce((memo, p) => [memo[0] + p[0] / 3, memo[1] + p[1] / 3], [0, 0])
      const c = positions.reduce((memo, p) => [memo[0] + p[0] / 3, memo[1] + p[1] / 3], [0, 0])
      const pts = positions.map(p => {
        const cp = vec2.sub([], p, c)
        return vec2.scaleAndAdd([], c, cp, t.triScale.getCurrentValue())
      })
      ...
      if (f - 30 > settings.speed * 3 * (rand.noise2D(center[0] + 99999, center[1], settings.animationNoise * 0.1) * 0.5 + 0.5)) {
        t.thirdPtT.setDestination(1)
      } else {
        t.thirdPtT.setDestination(0)
      }
      t.thirdPtT.tick()
