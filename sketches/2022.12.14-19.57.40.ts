...
const PLOTNAME = '2022.12.14-19.57.40'
...
canvasSketch(({ render }) => {
  ...
  gui.add(settings, 'forceAmount', 0, 1).step(0.01).onChange(render)
  gui.add(settings, 'maxIterations', 0, 1000).step(1).onChange(render)
  ...
  return (args: SketchArgs) => {
    ...
    for (let n = 0; n < settings.fractures; n++) {
      ...
      const idx = shapes.length - (rand.chance(0.5) ? 1 : rand.rangeFloor(shapes.length))
      const parentShape = shapes[idx]
      if (!parentShape) {
        n--
        continue
      }
      const line = getRandomLine()
      const newShapes = divideShapeWithLine(line, parentShape)
      if (newShapes === null) {
        retries += 1
        n--
        continue
      ...
      shapes.splice(idx, 1, ...newShapes.map(newShape => {
        ...
        // const dir = vec2.sub([], getCentroid(newShape), [0, 0])
        const dir = vec2.sub([], getCentroid(newShape), getCentroid(parentShape))
        vec2.normalize(dir, dir)
        const offset = vec2.scale([], dir, mag)
        moveShape(newShape, offset)
        return newShape
      }))
    }

    const layout = new ForceDirectedLayout<Shape>(
      shapes, getCentroid, getForce, moveShape
    )

    let wasUpdated = true
    let iterations = 0
    while (wasUpdated && iterations < settings.maxIterations) {
      wasUpdated = layout.tick()
      iterations += 1
    }
...
function distRayToShape (ray: Ray, shape: Shape): number | null {
  ...
  for (const int of intersections) {
    const dist = vec2.squaredDistance(pt, int)
    min = Math.min(min, dist)
  }
  return Number.isFinite(min) ? Math.sqrt(min) : null
}
...
function getForce(a: Shape, b: Shape): number {
  const dist = distanceBetweenShapes(a, b)
  const minDist = settings.minDist
  const delta = (dist - minDist) * settings.forceAmount
  return Math.min(delta, 0) // don't push any of them closer to each other
}

function distanceBetweenShapes(a: Shape, b: Shape): number {
  const aCentroid = getCentroid(a)
  const bCentroid = getCentroid(b)
  const dir = vec2.sub([], bCentroid, aCentroid)
  vec2.normalize(dir, dir)
  const abRay: Ray = [aCentroid, dir]
  const baRay: Ray = [bCentroid, vec2.scale([], dir, -1)]
  const dist = vec2.distance(aCentroid, bCentroid) - distRayToShape(abRay, a)! - distRayToShape(baRay, b)!
  return dist
}
...
class ForceDirectedLayout<T> {
  constructor(
    public nodes: T[],
    private getPosition: (node: T) => Vec2,
    private getForce: (node1: T, node2: T) => number,
    private applyUpdate: (node: T, update: Vec2) => void
  ) {}
  // TODO: speed all this up with a btree or something
  tick(): boolean {
    let updated = false
    const positions = new Map<T, Vec2>()
    const updates = new Map<T, Vec2>()
    for (const node of this.nodes) positions.set(node, this.getPosition(node))
    for (let i = 0; i < this.nodes.length; i++) {
      const node1 = this.nodes[i]
      for (let j = i + 1; j < this.nodes.length; j++) {
        const node2 = this.nodes[j]
        if (node1 === node2) continue // shouldn't ever happen
        const p1 = positions.get(node1)
        const p2 = positions.get(node2)
        const vec1to2 = vec2.sub([], p2, p1)
        const dir = vec2.normalize([], vec1to2)
        const mag = this.getForce(node1, node2)
        if (!updates.has(node1)) updates.set(node1, [0, 0])
        if (!updates.has(node2)) updates.set(node2, [0, 0])
        const update1 = updates.get(node1)
        const update2 = updates.get(node2)
        vec2.scaleAndAdd(update1, update1, dir, mag)
        vec2.scaleAndAdd(update2, update2, dir, mag * -1)
      }
    }
    for (const node of this.nodes) {
      const update = updates.get(node)
      if (update && update[0] * update[0] + update[1] * update[1] > 0) {
        this.applyUpdate(node, update)
        updated = true
      }
    }
    return updated
...
function moveShape(shape: Shape, offset: Vec2): void {
  for (const segment of shape) {
