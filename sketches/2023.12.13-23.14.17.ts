...
const PLOTNAME = '2023.12.13-23.14.17'
...
const settings = {
  seed: 6611,
  boxes: 35,
  minMargin: 105, // minimum margin around the canvas
  minHeight: 3, // each box must be at least this tall (in lineHeight units)
  minWidth: 20, // each box must be at least this wide
  lineHeight: 12, // each box height must be divisible by this number
  boxMargin: 1.5, // margin between boxes (in lineHeight units)

  // handwriting settings
  lineDivisions: 6,
  maxControlPtPerturb: 1,
  perturbNoiseFreq: 0.3,
  perturbNoiseMag: 8.5,
  perturbDivisionSize: 20,
...
canvasSketch(({ render }) => {
  ...
  return (args: SketchArgs) => {
    ...
    settings.boxes = rand.rangeFloor(5, 60)
    settings.minHeight = Math.floor(Math.pow(rand.value(), 6) * 15 + 2)
    settings.lineHeight = Math.floor(Math.pow(rand.value(), 3) * 10 + 5)
    ...
    for (const box of boxes) {
      ...
      if (rand.chance(0.7)) {
        if (rand.chance(0.1)) {
          lines.push(...boxEmpty(box))
        } else {
          lines.push(...handwriting(box, rand))
        }
      } else {
        if (rand.chance(0.1)) {
          lines.push(...boxFill(box, 3, rand))
        } else if (rand.chance(0.2)) {
          lines.push(...boxFill(box, 2, rand))
        } else {
          lines.push(...boxFill(box, 1, rand))
        }
      }
    }
...
function boxEmpty(box: Box): Line[] {
  return [[
    [box.x, box.y],
...
function boxFill(box: Box, passes: number, rand: random.RandomGenerator): Line[] {
  const lines: Line[] = [[
    [box.x, box.y],
    [box.x + box.width, box.y],
    [box.x + box.width, box.y + box.height],
