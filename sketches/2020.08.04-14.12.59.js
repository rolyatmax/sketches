...
function setup () {
  ...
  draw = rico({
    ...
    void main() {
      float colorT1 = noise3D(position * colorVariance + vec3((time + randomVal * 10.0) * 0.1, 0, 0), 1.0, vec2(0, 1));
      float colorT2 = noise3D(position * colorVariance * 2.0 + vec3((time + randomVal * 100.0) * 0.1, 50, 980), 1.0, vec2(0, 1));
      float colorT3 = noise3D(position * colorVariance * 1.5 + vec3((time + randomVal * 10.0) * 0.2, 99999, 2), 1.0, vec2(0, 1));
      float colorT4 = noise3D(position * colorVariance * 0.5 + vec3((time + randomVal * 6.0) * 0.6, 0, 200), 1.0, vec2(0, 1));
      float colorT5 = noise3D(position * colorVariance * 2.5 + vec3((time + randomVal * 2.0) * 0.05, 999, -51), 1.0, vec2(0, 1));
      float colorT = min(max(min(max(colorT1, colorT2), colorT3), colorT4), colorT5);
      colorT *= 1.3;
      colorT -= 0.15;
    ...
    const vec3 lightDir2 = normalize(vec3(-5, 1, -4));
    ...
    void main() {
      ...
      // float z = pow(1.0 - xySquared, 0.5);
      // vec3 normal = normalize(vec3(vQuadPosition, z));
      // normal = (view * vec4(normal, 1)).xyz;
      // float shade1 = clamp(dot(normal, lightDir1), 0.0, 1.0);
      // float shade2 = clamp(dot(normal, lightDir2), 0.0, 1.0);
      // fragColor = vec4(vColor.rgb + max(shade1, shade2) * 0.15, vColor.a);
    }
    `)
  })
}
  ...
  return ({ width, height, time }) => {
    ...
    draw({
      uniforms: {
        ...
        projection: mat4.perspective([], Math.PI / 2, width / height, 0.01, 1000),
        time: time,
        rand: randomVal,
