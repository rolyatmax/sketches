...
const sketch = ({ render }) => {
  ...
  gui.add(settings, 'triMargin', 0, 2).step(0.01).onChange(render)
  ...
  function createCircles () {
    ...
    for (let i = 0; i < triangulatedPositions.length; i += 3) {
      const positions = []
      positions.push(pts[triangulatedPositions[i]])
      positions.push(pts[triangulatedPositions[i + 1]])
      positions.push(pts[triangulatedPositions[i + 2]])

      const center = positions.reduce((memo, p) => [memo[0] + p[0] / 3, memo[1] + p[1] / 3], [0, 0])
      const points = positions.map(pos => ({
        position: pos,
        normDir: vec2.normalize([], vec2.sub([], pos, center)),
        leng: vec2.distance(center, pos)
      }))

      triangles.push({ id: i / 3, points, center })
    }
  ...
  return ({ context, width, height }) => {
    ...
    for (const t of triangles) {
      ...
      const chance1 = Math.abs(rand.noise2D(t.center[0] + 900, t.center[1] + 900, 0.0005))
      if (rand.chance(chance1 * settings.colorChance)) {
        const val1 = rand.noise2D(t.center[0], t.center[1], settings.colorNoiseFreq)
        const val2 = rand.noise2D(t.center[0] + 100000, t.center[1], settings.colorNoiseFreq * 1.1)
        const idx1 = settings.colors * (val1 * 0.5 + 0.5) | 0
        const idx2 = settings.colors * (val2 * 0.5 + 0.5) | 0

        const a1 = Math.abs(rand.noise2D(t.center[0] + 500, t.center[1], 0.0003))
        const a2 = Math.abs(rand.noise2D(t.center[0] + 500000000, t.center[1], 0.0005))
        const chance2 = rand.chance(rand.noise2D(t.center[0] + 999999, t.center[1], 0.0002) * 0.5 + 0.5) ? a1 : a2

        const color = rand.chance(chance2) ? colors[idx1] : colors[idx2]
        const alpha = rand.noise2D(t.center[0], t.center[1] - 9876, 0.0007) * settings.alpha + settings.alpha
